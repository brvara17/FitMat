...
public class BluetoothCommService {
    ...
    private static class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    FindUseFullData(buffer);
                    buffer = new byte[512];

                    //Log.d(BluetoothCommService.TAG, String.valueOf(buffer));
                } catch (IOException e) {
        ...
        public void FindUseFullData(byte[] buffer)
        {
            int StartSequence = 0;
            for(int i=0; i<512;i++)
            {
                Log.d("Buffer Data ", String.format("%02x", buffer[i]));
                if(buffer[i] == 0x4d)
                {
                    StartSequence = 1;
                }
                else if(StartSequence == 1 && buffer[i] == 0x00) StartSequence = 2;
                else if (StartSequence == 2 && buffer[i] == 0x0A) StartSequence = 3;
                else StartSequence = 0;

                if(i == 511)
                {
                    Log.d("Full Byte Array", "Showing All above");
                }

                if(StartSequence == 3)
                    Log.d("Found Start Sequence", "Next Byte is " + buffer[i+1]);
            }
        }
